...
当初はFisher判別分析（LDA）を代替の局所サロゲートとして提案・検証する計画だったが、実証の結果（フェーズ6・7、`docs/EXPERIMENT_LOG.md`参照）Fisherは分析章に位置づけを変更した。**現在は、多クラスBBの予測上位2クラスに絞り、その2クラスを区別する局所説明を研究対象とする。**説明点で選んだペア$(c^*,c')$について、2クラスの確率の合計が1になるよう$q=p_{c^*}/(p_{c^*}+p_{c'})$とし、通常LIME、log-odds Ridge、local logisticを適用して比較する。この確率調整自体には独自名称を付けない。CLIMAXはlog-odds Ridge／local logisticの最重要先行研究だが、2クラスに絞る処理は説明器から独立してLIME等にも適用できるため、研究全体をCLIMAX拡張には限定しない。中心的な貢献候補は、OVR型説明と特定2クラスの説明の違いを整理し、同一の摂動・特徴予算・held-out評価で、2クラスに絞る効果とリンク関数／損失の効果を分離して検証することである。経緯の詳細は`docs/EXPERIMENT_LOG.md`、手法の定式化一覧は`docs/OVO_LIME_METHODS.md`を参照。
...
中間発表の現行デッキは`pptx/MIDTERM_PRESENTATION_v3.pptx`（30枚、唯一の正本。他のpptxファイルは削除済み）。導入・目次・関連研究・提案手法のパートに加え、結果パートは(1) 非線形・相関・ノイズを含む局所真値Top-5特徴再現、(2) 同条件のheld-out符号忠実度、(3) クラス数増加時の計算時間、(4) まとめと今後の展望、の4枚構成。線形softmaxのSpearman 0.9993と、競合特徴数=Kで約1.0となった旧特徴役割実験は、答えが簡単すぎる整合性確認として主結果から外している。全スライド（LIMEアルゴリズムのビルドアニメーション複製と非表示スライドを除く）に12分想定のスピーカーノートを設定済み。
...
- **CLIMAXとの位置づけを明確化（2026-09-08）**：CLIMAXは対象クラスの$\log(p_c/(1-p_c))$を局所Ridgeで近似するL-CLIMAXと、確率をcross entropyで近似するCE-CLIMAXを提案しており、今回のContrastive Ridge／OVO Logisticに最も近い。ただし今回は、restを一括した$1-p_c$ではなく、特定の競合クラス$p_d$だけを比較対象とする。CLIMAXは最重要先行研究・比較対象として扱うが、研究全体をCLIMAX拡張とは呼ばない。二値分類では$q_{c,d}=p_c$となるためCLIMAX型との違いは消え、研究対象は3クラス以上である。
...
- 実測で確認済みの結論（詳細は`docs/RECENT_WORK.md`参照）：
  ...
  - **研究ストーリーの組み替え（2026-09-05、ユーザー承認済み・確定方針）**：Fisherは提案から降ろし分析章に位置づけ、Contrastive LIMEを提案手法とする。降ろした後「分析だけでなく新しい手法を出す」ため、Contrastiveの改良案を4つ試した（詳細は`docs/EXPERIMENT_LOG.md`フェーズ8〜11）。
    ...
    - **フェーズ12/12.5（2026-09-08〜09、中心的な問いの直接検証・再訂正）**：従来の疎版は、Lasso/L1モデルを特徴選択だけでなく最終係数にも使い、通常LIMEの「特徴選択後の最終サロゲート再フィット」を欠いていた。全方式をselect-then-refitへ修正し、上位2クラスに絞った$q=p_{c_1}/(p_{c_1}+p_{c_2})$へ通常LIMEを適用するベースラインと、表示特徴数を厳密にKへ揃えるOVR-exactを追加した。RF・held-out符号忠実度は2クラス通常LIME 0.7914、Contrastive 0.7889、OVR-exact 0.7873、C 0.7818。2クラス通常LIMEはOVR-exactより+0.0041（5/18セルでHolm有意）、Contrastiveより+0.0024、Cより+0.0096だった。**このRF・疎設定では2クラスを選んで1本の説明にする効果が中心で、logit変換やcross-entropyの追加優位は確認できなかった**。旧+0.045/+0.0405は再フィット欠落の影響を含むため使わない。
    - **黒箱構造の追加比較（2026-09-09）**：線形softmax、非線形NN＋softmax、RFを同じexact-K・held-out設計で比較した。線形softmaxではContrastiveが最良（0.8655、2クラス通常LIME 0.8632、OVR 0.8492）だったが、非線形softmaxでは2クラス通常LIME 0.7840がContrastive 0.7808を、RFでは0.7847が0.7819を上回った。**softmaxであること自体は対数比の優位を保証せず、2クラスのlogit差が入力に対して局所線形に近いことが条件**と解釈する。
    - **特徴役割を既知にした解釈性実験（2026-09-09）**：線形softmax BBにA/B競合特徴、A/B共通特徴、他クラス専用・無関係特徴を明示した。共通特徴を強くすると通常のtop-class LIMEの競合特徴recallは0.998→0.780、OVR-exactは0.996→0.807へ低下し、表示特徴の約19〜22%をA/B共通特徴に使った。2クラス通常LIME・Contrastive・OVO Logisticは競合特徴recallをほぼ1.0、共通特徴選択率をほぼ0に維持した。これにより、2クラス説明の解釈性上の利点を「A/Bの違いに関係する特徴を表示できるか」として定量評価できた。
    ...
    - **最新の採用方針（2026-09-09、exact-K・追加BB実験を反映）**：研究の主軸は、予測1位・2位を対象にする局所説明の枠組みとする。汎用的な第一候補は2クラスを選んで適用する通常LIME。2クラスのlogit差が局所線形に近い場合はContrastiveが有力で、OVO Logisticは確率校正または正則化改善の発展案とする。提案C単体を主軸とした以前の方針は撤回する。Fisherは機構分析に位置づける。
  - **グラウンドトゥルース検証（Rahnama et al. 2024型、2026-09-09更新）**：多項ロジスティック回帰の真の対数オッズ係数$\theta_{c^*}-\theta_{c'}$とのSpearman順位相関を測定。全グリッド平均はContrastive 0.9993、OVO Logistic 0.9975、2クラス通常LIME 0.9872、Fisher(soft) 0.9832、OVR 0.9758、Fisher(hard) 0.9528。ContrastiveとOVO Logisticはいずれも2クラス通常LIMEを全9セルで有意に上回った。線形softmaxでは対数オッズを直接当てはめる設計が真の特徴方向を最も正確に復元する。
...
- `src/surrogates.py`: サロゲートフィッティング関数群。
  ...
  - `fit_pairwise_lime` / `fit_pairwise_lime_lasso`: 上位2クラスの$q=p_{c1}/(p_{c1}+p_{c2})$へ通常の重み付きRidge／Lasso選択を適用する比較用実装。
  - `fit_ovo_logistic`: 同じ$q$をソフトラベル交差エントロピーで当てはめるlocal logistic実装。
...
- `src/stats_utils.py`: 実験ドライバ共通の統計ヘルパー。`bootstrap_ci`（パーセンタイル・ブートストラップ信頼区間）、`paired_wilcoxon`（対応ありWilcoxon符号順位検定＋matched-pairs rank-biserial効果量）、`holm_bonferroni`（多重比較補正）、`compare_methods`（グリッドセルごとにseedレベル平均を独立サンプルとして扱う高水準関数）。Holm補正は各手法ペアについて全グリッドセルを1つの検定族として適用する。擬似反復を避ける理由はモジュールのdocstringに記載。
...
- `src/run_groundtruth_experiment.py`: Rahnama et al. (2024)型のグラウンドトゥルース検証。`LogisticRegression(multinomial)`の真の対数オッズ係数とOVR / Fisher / 2クラス通常LIME / Contrastive / OVO Logisticの推定係数のSpearman順位相関を測る。`N_DATASET_SEEDS=20`で反復。
...
- **最優先**：提案Cが密な設定（フェーズ11.5）と疎な設定（フェーズ12.5）で逆の結論になる原因をさらに調査する。特徴選択後の再フィット欠落は2026-09-08に修正し、旧疎版の極端な縮小は除去できたが、公平な再フィット後もCはContrastiveより忠実度−0.0071、pairwise LIMEより−0.0096だった。残る候補はL1で選ばれる支持集合の差、$C=1$の正則化強度、サンプル数・局所幅との相互作用。
- OVR-exactは候補和集合を係数差で順位付けする方式で実装済み。別のK配分規則でも結論が維持されるかを確認する。
- 共通特徴の回避は既知の特徴役割を持つ合成BBで検証済み。競合クラスが入力ごとに変わる場合の追随性は未検証。
