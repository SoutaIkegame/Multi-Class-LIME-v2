...
## フェーズ12.7b: 非線形局所グラウンドトゥルース再実験（2026-09-10）

**目的**：線形softmaxでContrastiveのSpearmanが0.9993、競合特徴3個・K=3の人工設定で再現率がほぼ1.0となった旧結果は、数式上正解が見えやすい。そこで、真の説明が点ごとに変わる難しい条件で、特徴関連性とheld-out忠実度を再評価した。

**設計**：30次元の滑らかな非線形softmax BBを各seedで独立生成。入力候補は相関0.55のGaussian、スコアにはクラス共通効果、A/B固有効果、弱い密ノイズ、sin・交互作用・二次項を含めた。C=3/5/8、曲率=0.35/0.80、K=5、20独立BB、各6説明点、学習用・評価用各400摂動。正解は各説明点における$\nabla_x\log(p_A/p_B)$を中心差分で数値計算し、その絶対値Top-5とした。全方式は同じ点・摂動・重み・Kを使用した。

**結果（全条件平均）**：

| 手法 | 真の局所Top-5再現率 | 係数方向cosine | held-out符号忠実度 |
|---|---:|---:|---:|
| OVR-exact | 0.8203 | 0.9374 | 0.8871 |
| 2クラス通常LIME | 0.9128 | 0.9498 | 0.8983 |
| Contrastive | **0.9261** | **0.9509** | **0.9005** |
| OVO Logistic | 0.8742 | 0.9449 | 0.8941 |

2クラス通常LIMEとContrastiveは、Top-5再現率・cosine・符号忠実度のすべてでOVRを6/6条件でHolm補正後も有意に上回った。特徴再現率の改善は約9.2〜10.6ポイントで、旧実験の1.0張り付きは解消した。符号忠実度の改善は約1.1〜1.3ポイントであり、統計的には一貫するが実用差は小さい。

**出力**：`src/run_challenging_groundtruth_experiment.py`、`src/plot_challenging_groundtruth.py` → `results/challenging_groundtruth_{results,stats,summary}.csv`、`results/challenging_groundtruth_{comparison,recall,fidelity}.png`。

**解釈**：本実験で最も強い結果は予測値の大幅改善ではなく、限られたK個の表示枠へ真のA/B局所特徴を集められたことである。人工的な滑らかなsoftmax BBであるため、実データ・非softmax BBへの一般化は未確認。
