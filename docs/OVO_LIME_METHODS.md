...
**最新の実験訂正（2026-09-09）**：上位2クラスを選んで$q=p_c/(p_c+p_d)$に通常LIMEを適用するベースラインを追加し、全疎手法を「特徴選択→最終モデル再フィット」、表示特徴数exact-Kへ統一した。RFでは2クラス通常LIME 0.7914、Contrastive 0.7889、OVR-exact 0.7873、OVO Logistic 0.7818だった。追加BB比較では、線形softmaxのみContrastiveが最良（0.8655）で、非線形NN＋softmaxとRFでは2クラス通常LIMEが最良だった。したがって、2クラスを選ぶ効果は比較的一貫している一方、logit変換の追加利得は2クラスのscore差が局所線形に近い場合に限られる可能性が高い。旧フェーズ12の+0.0453/+0.0405は再フィット欠落の影響を含むため使用しない。詳細は`docs/EXPERIMENT_LOG.md`フェーズ12.5・12.6を参照。
